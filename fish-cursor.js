...
class WebGLFishCursor {
     ...
     * @param {Object} [options.configOverrides={}] - Override default config values (see config properties below)
     ...
     * 
     * @property {Object} config - Configuration object with the following properties:
     * @property {number} config.FISH_COLOR - Default fish body color (hex)
     * @property {number} config.EYE_COLOR - Fish eye color (hex)
     * @property {number} config.FOLLOW_EASE - Easing factor for fish following pointer (0-1)
     * @property {number} config.SCALE - Fish size multiplier (0.8 = 80% of base size)
     * @property {Object} config.COLOR_SLOW - RGB values for fish color when moving slowly {r, g, b}
     * @property {Object} config.COLOR_FAST - RGB values for fish color when moving fast {r, g, b}
     * @property {number} config.PARTICLE_SPAWN_RATE - Background particles spawned per second
     * @property {string[]} config.PARTICLE_COLORS - Array of hex color strings for particles
     * @property {number} config.SMOOTHING - Movement smoothing factor (higher = smoother/slower)
     * @property {number} config.STAR_GRID_SIZE - Grid dimension for star placement (1-4, creates NxN grid)
     * @property {number} config.STAR_UI_LEFT_PX - Left margin in pixels to avoid UI overlap
     * @property {number} config.STAR_FLOAT_RADIUS - Radius of star floating animation
     * @property {string[]} config.STAR_COLORS - Array of hex color strings for stars
     */
